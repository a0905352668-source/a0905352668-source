...
def extract_people(pose_result, kp_conf: float, width: int, height: int) -> list[PersonPose]:
    ...
    for i, kp in enumerate(pose_result.keypoints.data.cpu().numpy()):
        ...
        people.append(PersonPose(i, pts_all[0], pts_all[1], pts_all[2], pts_all[3], pts_all[4], pts_all[5], pts_all[6], box, scale))
    return people
...
def classify_candidate(phone: PhoneCandidate, static_zone_score: float, person_match_score: float, hand_link_score: float, aim_score: float) -> str:
    if phone.conf >= 0.55 and static_zone_score >= 0.8 and person_match_score >= 0.7 and hand_link_score >= 0.8 and aim_score >= 0.7:
        return "strong"
    if phone.conf >= 0.40 and static_zone_score >= 0.5 and person_match_score >= 0.5 and aim_score >= 0.5:
        return "normal"
    return "weak"


def evaluate_phone(phone: PhoneCandidate, screen: ScreenConfig, persons: list[PersonPose], width: int, height: int) -> CandidateEval:
    required_conf = screen.params["phone_valid_conf_person_roi"] if phone.source in ("person_roi", "person_screen_roi") else screen.params["phone_valid_conf_near_zone"]
    ...
    if zone_reason.startswith("ignore"):
        ...
        person, person_match_score, hand_score, person_roi, hand_rois, corridor, _corridor_hit = match_phone_to_person(phone, related_persons, screen, width, height)
        if person is None:
            ...
            risk = (
                0.25 * normalize_conf(phone.conf)
                + 0.20 * static_score
                + 0.25 * person_match_score
                + 0.20 * aim_score
                + 0.10 * source_score
            )
            level = classify_candidate(phone, static_score, person_match_score, hand_score, aim_score)
            candidate_ok = phone.conf >= required_conf and static_score >= 0.5 and person_match_score >= 0.45 and aim_score >= 0.5
            if not candidate_ok:
                reject = "low_joint_score"
            candidate_reason = f"{level}|{zone_reason}|aim={aim_reason}|risk={risk:.2f}"
    return CandidateEval(phone, screen, person, static_score, zone_reason, person_match_score, hand_score, aim_score, source_score, risk, level, reject, candidate_reason, best_angle, best_ray_hit, person_roi, hand_rois, corridor)
    ...
    writer.writerow(
        {
            ...
            "person_id": "" if p is None else p.index,
...
def draw_outputs(frame, screens: list[ScreenConfig], people: list[PersonPose], evals: list[CandidateEval], alert_counter: float, stable_alert: bool, draw_static_zones: bool) -> None:
    for screen in screens:
        if draw_static_zones:
        ...
        draw_poly(frame, screen.screen_poly, (255, 80, 0), 2, "SCREEN")
    for person in people:
        draw_rect(frame, person.box, (80, 180, 80), 1, f"P{person.index}")
    ...
    for ev in evals:
        ...
        if ev.accepted and ev.level == "strong":
            ...
            color = (180, 0, 180)
        draw_rect(frame, ev.phone.box, color, 3 if ev.accepted else 1, f"{ev.phone.source}:{ev.phone.conf:.2f}")
        ...
        if ev.accepted and ev.person is not None:
    ...
    status = "STABLE_ALERT" if stable_alert else "OK"
    color = (0, 0, 255) if stable_alert else (60, 180, 60)
    draw_label(frame, f"v2.1 {status} alert_counter={alert_counter:.1f}", 12, 38, color, scale=0.7)
...
def main() -> None:
    ...
    threshold = max(float(s.params["alert_counter_threshold"]) for s in screens) if screens else 10.0
    alert_counter = 0.0
    last_good = None
    stats = {
        "frames": 0,
    ...
    with debug_csv.open("w", newline="", encoding="utf-8") as f:
        ...
        while True:
            ...
            if accepted:
                alert_counter += max(candidate_delta(e.level) for e in accepted)
            else:
                alert_counter -= 1.5
            alert_counter = clamp(alert_counter, 0.0, 20.0)
            stable_alert = alert_counter >= threshold

            for ev in evals:
                write_debug_row(csv_writer, frame_id, ev, alert_counter, stable_alert, ev.effective_search_roi)

            annotated = frame.copy()
            draw_outputs(annotated, screens, people, evals, alert_counter, stable_alert, args.draw_static_zones)
            ...
            if frame_id % 100 == 0 or frame_id == total:
                print(
                    ...
                    f"alert={alert_counter:.1f} gray_fixed={stats['gray_frames_replaced']}",
                    flush=True,
                )
