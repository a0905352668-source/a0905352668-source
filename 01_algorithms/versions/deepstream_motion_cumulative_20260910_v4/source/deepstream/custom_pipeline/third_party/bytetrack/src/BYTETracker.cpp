...
BYTETracker::BYTETracker(int frame_rate, int track_buffer, float detection_threshold)
{
	track_thresh = detection_threshold;
	high_thresh = detection_threshold;
	match_thresh = 0.8;
...
vector<STrack> BYTETracker::update(const vector<Object>& objects)
{
	...
	this->frame_id++;
	// Expire BEFORE association, so an expired identity cannot be resurrected
	// by a matching detection on the timeout boundary. No historical tombstones.
	this->removed_stracks.clear();
	this->lost_stracks.erase(std::remove_if(this->lost_stracks.begin(), this->lost_stracks.end(),
		[this](const STrack& t) { return this->frame_id-t.frame_id > max_time_lost; }),
		this->lost_stracks.end());
	...
	for (int i = 0; i < u_detection.size(); i++)
	{
		...
		if (track->score < this->high_thresh)
			continue;
		track->activate(this->kalman_filter, this->frame_id, ++next_track_id);
		activated_stracks.push_back(*track);
	}
