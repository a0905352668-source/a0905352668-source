...
int main() {
    ...
    GatedEventPolicy recent_motion_candidate;
    recent_motion_candidate.set_infer_fps(10.0);
    for (int i = 0; i < 13; ++i) {
        auto item = strong();
        item.person_motion_candidate = i == 11;
        decision = recent_motion_candidate.update(item);
    }
    require(!decision.alarm,
            "a motion trend one frame before alarm eligibility must delay publication");
    require(decision.reject_reason == "person_motion_confirmation",
            "recent motion trend must expose the confirmation reason");
    for (int i = 0; i < 4; ++i) {
        decision = recent_motion_candidate.update(strong());
        require(!decision.alarm,
                "unconfirmed motion trend must wait for the full half second");
    }
    decision = recent_motion_candidate.update(strong());
    require(decision.alarm,
            "an unconfirmed motion trend must not delay a valid event beyond half a second");
