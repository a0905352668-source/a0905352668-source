...
struct PersonTrackState {
    ...
    jiankong::custom_pipeline::LargeMotionPolicy person_motion_policy;
...
struct StreamState {
    ...
    std::unique_ptr<jiankong::custom_pipeline::PersonByteTracker> person_tracker;
...
static void assign_bytetrack_ids(std::vector<Det>& people, const std::vector<Det>& low,
    jiankong::custom_pipeline::PersonByteTracker& tracker,
    std::map<int,PersonTrackState>& states, int frame_id,
    const jiankong::custom_pipeline::SpatialStaticConfig& config, bool spatial_enabled) {
    std::vector<Object> input;
    const auto append = [&](const Det& d) {
        input.push_back({cv::Rect_<float>(d.box.x1,d.box.y1,d.box.w(),d.box.h()),0,d.conf});
    };
    for(auto& d: people) { d.track_id=-1; append(d); }
    for(const auto& d: low) append(d);
    std::set<int> visible;
    for(const auto& match: tracker.update(input)) {
        if(match.input_index>=static_cast<int>(people.size())) continue;
        auto& person=people[match.input_index];
        person.track_id=match.track_id;
        auto it=states.find(match.track_id);
        // An interrupted evidence chain must be reacquired, even if identity survives.
        if(it!=states.end() && it->second.last_seen!=frame_id-1) states.erase(it);
        auto [current,inserted]=states.try_emplace(match.track_id,config);
        auto& state=current->second;
        if(inserted) state.spatial_static_enabled=spatial_enabled;
        state.track_id=match.track_id;
        state.last_seen=frame_id;
        state.last_bbox=person.box;
        state.has_last_bbox=true;
        visible.insert(match.track_id);
    }
    // Kalman predictions/low-score matches never keep business alarms alive.
    for(auto it=states.begin();it!=states.end();) {
        if(!visible.count(it->first)) it=states.erase(it); else ++it;
    }
}
...
static void update_person_motion(PersonTrackState& state, const Det& person,
                                 int frame_id, double infer_fps) {
    const bool valid = person.kpts[5][2] >= .35f && person.kpts[6][2] >= .35f;
    state.person_motion_policy.update(frame_id / std::max(1.0, infer_fps),
        (person.box.x1+person.box.x2)*.5, (person.box.y1+person.box.y2)*.5,
        (person.kpts[5][0]+person.kpts[6][0])*.5,
        (person.kpts[5][1]+person.kpts[6][1])*.5, person.box.h(), valid);
}
...
static void update_person_alarm_state(
    ...
    update_person_motion(state, person, frame_id, infer_fps);
...
static std::vector<Det> decode_pose(const TrtRunner& runner, int batch_index, int channels,
                                    ...
                                    float min_conf=0.f, float max_conf=2.f) {
    ...
    for (int slot : slots) {
        ...
        if(!std::isfinite(conf) || conf<min_conf || conf>=max_conf) continue;
        Det d;
        d.conf = conf;
...
struct Args {
    ...
    std::string person_tracker = "bytetrack";
    float kp_conf = 0.35f;
...
static jiankong::custom_pipeline::GatedFrameEvidence gated_frame_evidence(
    ...
    evidence.person_moving = state.person_motion_policy.moving();
    evidence.person_motion_candidate = state.person_motion_policy.candidate();
    if (ev != nullptr) {
...
static Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (k == "--person-tracker") a.person_tracker = next();
    ...
    if(a.person_tracker!="legacy" && a.person_tracker!="bytetrack")
        throw std::runtime_error("--person-tracker must be legacy or bytetrack");
    if(a.person_tracker=="bytetrack" && (!std::isfinite(a.pose_conf) ||
        a.pose_conf<.10f || a.pose_conf>1.f || !std::isfinite(a.infer_fps) || a.infer_fps<1))
        throw std::runtime_error("ByteTrack requires pose confidence [0.10,1] and FPS >= 1");
    return a;
}
...
static void write_event_metadata(std::ostream& jsonl,
    ...
    for (const auto& person : people) {
        ...
        p["person_moving"] = st != nullptr && st->person_motion_policy.moving();
...
static int run_rule_self_test() {
    ...
    // 12px/frame at 8 FPS for a 340px person is continuous translation
    // (~28%/s).  It stays below the one-second large-motion threshold but
    // must now be suppressed after the longer cumulative window confirms it.
    if (!handheld_replayed || !handheld_state.static_shadow_history.empty() ||
        handheld_state.alarm_triggered ||
        handheld_state.gated_event_decision.reject_reason != "person_moving" ||
        handheld_state.window_hits != 0 || handheld_state.legacy_window_hits != 24 ||
    ...
    std::map<int, PersonTrackState> large_motion_states;
    for (int frame_id=0; frame_id<24; ++frame_id) {
        const float dx=frame_id*24.0f;
        std::vector<Det> movers{shifted_pose_person(person, dx, 4)};
        movers[0].track_id=handheld_track_id;
        std::vector<CandidateEval> evals{shifted_risky_eval(dx,handheld_track_id,4)};
        update_person_states(movers,evals,large_motion_states,frame_id,
            screen.params.person_state_window,screen.params.person_state_risk_threshold,
            screen.params.person_state_min_hits,screen.params.handheld_suspect_min_hits,8.0);
    }
    const auto& large_motion_state=large_motion_states[handheld_track_id];
    if (large_motion_state.alarm_triggered || large_motion_state.window_hits!=0 ||
        large_motion_state.gated_event_decision.reject_reason!="person_moving") {
        std::cerr<<"[SELF_TEST_RULES] large movement must clear alarm evidence"<<std::endl;
        return 32;
    }
    ...
    // Exercise both real per-person update branches, not just the gate helper.
    for (bool spatial_enabled : {false, true}) {
        std::map<int, PersonTrackState> moving_states;
        bool resumed = false;
        for (int f=0; f<90; ++f) {
            const float dx = std::min(f, 39)*16.0f;
            std::vector<Det> moving_people{shifted_pose_person(person, dx, 0)};
            moving_people[0].track_id=901;
            std::vector<CandidateEval> moving_evals{shifted_risky_eval(dx,901,0)};
            update_person_states(moving_people,moving_evals,moving_states,f,30,
                screen.params.person_state_risk_threshold,10,10,10.0,
                nullptr,0.0,nullptr,spatial_enabled);
            const auto& ms=moving_states.at(901);
            if(f>=10 && f<40 && (ms.alarm_triggered || ms.gated_event_decision.review)) {
                std::cerr << "[SELF_TEST_RULES] moving person was reported" << std::endl;
                return 31;
            }
            if(f>65 && ms.alarm_triggered) resumed=true;
        }
        if(!resumed) {
            std::cerr << "[SELF_TEST_RULES] stopped person failed to rearm" << std::endl;
            return 32;
        }
    }
    {
        jiankong::custom_pipeline::PersonByteTracker tracker;
        std::map<int,PersonTrackState> tracked;
        Det d; d.box=Rect{0,0,40,100}; d.conf=.9f;
        std::vector<Det> visible{d};
        assign_bytetrack_ids(visible,{},tracker,tracked,1,{},false);
        if(visible[0].track_id<0 || tracked.size()!=1) return 33;
        const int id=visible[0].track_id;
        tracked.at(id).alarm_triggered=true;
        tracked.at(id).phone_history.push_back(1);
        visible.clear(); d.conf=.15f;
        assign_bytetrack_ids(visible,{d},tracker,tracked,2,{},false);
        if(!tracked.empty()) return 34;
        d.conf=.9f; visible={d};
        assign_bytetrack_ids(visible,{},tracker,tracked,3,{},false);
        if(visible[0].track_id!=id || tracked.at(id).alarm_triggered ||
            !tracked.at(id).phone_history.empty()) return 35;
        jiankong::custom_pipeline::PersonByteTracker late_tracker;
        tracked.clear(); visible.clear();
        assign_bytetrack_ids(visible,{},late_tracker,tracked,1,{},false);
        visible={d};
        assign_bytetrack_ids(visible,{},late_tracker,tracked,2,{},false);
        if(visible[0].track_id!=-1 || !tracked.empty()) return 36;
        assign_bytetrack_ids(visible,{},late_tracker,tracked,3,{},false);
        if(visible[0].track_id<0 || tracked.size()!=1) return 37;
    }
...
static void print_live_usage() {
    std::cout
        << "RTSP live mode:\n"
        << "  --person-tracker legacy|bytetrack  default bytetrack; legacy for rollback\n"
...
int main(int argc, char** argv) {
    try {
        ...
        std::cerr << "[PERSON_TRACKER] " << args.person_tracker << std::endl;
        ...
        while (true) {
            ...
            if (live_mode) {
                ...
                if (live_measurement_started_at == 0.0) {
                    ...
                    if (now_sec() >= live_startup_deadline) {
                        std::ostringstream startup_details;
                        startup_details << " ready_ids=";
                        for (const auto source_id : live_ready_sources) {
                            startup_details << source_id << ',';
                        }
                        startup_details << " missing=";
                        for (std::size_t source_id = 0; source_id < streams.size(); ++source_id) {
                            if (live_ready_sources.count(static_cast<unsigned int>(source_id)) == 0) {
                                startup_details << source_id << ':' << streams[source_id].view << ',';
                            }
                        }
                        throw std::runtime_error(
                            "live startup timeout before all configured sources produced a frame: ready=" +
                            std::to_string(live_ready_sources.size()) + startup_details.str());
                    }
                    continue;
                }
            ...
            pose.infer_compacted(live_mode || args.gpu_preprocess,
                                 ...
                                 pose_boxes,
                                 4,
                                 args.person_tracker=="bytetrack" ? .10f : args.pose_conf);
            ...
            for (int b = 0; b < static_cast<int>(active_streams.size()); ++b) {
                ...
                if(args.person_tracker=="bytetrack") {
                    if(!streams[si].person_tracker)
                        streams[si].person_tracker=std::make_unique<jiankong::custom_pipeline::PersonByteTracker>(
                            static_cast<int>(std::round(args.infer_fps)),args.pose_conf);
                    auto low=dedupe_people(decode_pose(pose,b,pose_channels,pose_metas[b],
                        streams[si].width,streams[si].height,args.kp_conf,.10f,args.pose_conf),args.kp_conf);
                    assign_bytetrack_ids(people,low,*streams[si].person_tracker,streams[si].person_states,
                        streams[si].frame_id,spatial_static_config,args.spatial_static_phone_suppression_enabled);
                } else {
        ...
        if (live_mode) {
            ...
            summary["person_tracker"] = args.person_tracker;
