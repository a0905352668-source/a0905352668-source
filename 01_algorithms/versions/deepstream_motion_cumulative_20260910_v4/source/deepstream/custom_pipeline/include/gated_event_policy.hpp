...
struct GatedFrameEvidence {
    bool person_moving = false;
    bool person_motion_candidate = false;
...
inline const char* gated_event_state_name(GatedEventState state) {
    switch (state) {
        case GatedEventState::moving_suppressed: return "PERSON_MOVING_SUPPRESSED";
...
public:
    ...
    GatedEventDecision update(const GatedFrameEvidence& evidence) {
        if (evidence.person_moving) {
            reset();
            GatedEventDecision decision;
            decision.state = GatedEventState::moving_suppressed;
            decision.reject_reason = "person_moving";
            return decision;
        }
        ...
        const bool alarm_ready = history_.size() >= scaled(10) &&
        ...
        if (motion_confirmation_remaining_ > 0) {
            --motion_confirmation_remaining_;
            if (motion_confirmation_remaining_ == 0) {
                motion_confirmation_completed_ = true;
            }
        }
        const bool recent_motion_candidate = any_last(scaled(2), [](const auto& item) {
            return item.person_motion_candidate;
        });
        if (alarm_ready && !motion_confirmation_completed_ &&
            motion_confirmation_remaining_ == 0 && recent_motion_candidate) {
            motion_confirmation_remaining_ = scaled(4);
        }
        if (alarm_ready && motion_confirmation_remaining_ > 0) {
            decision.state = GatedEventState::stable_aiming;
            decision.reject_reason = "person_motion_confirmation";
            return decision;
        }
        if (alarm_ready) {
            alarm_latched_ = true;
...
private:
    ...
    void clear_motion_confirmation() {
        motion_confirmation_remaining_ = 0;
        motion_confirmation_completed_ = false;
    }
    ...
    std::size_t motion_confirmation_remaining_ = 0;
    bool motion_confirmation_completed_ = false;
};
