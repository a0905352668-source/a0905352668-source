#!/usr/bin/env python3
"""Surveillance anti-filming inference v2.3 with static phone suppression.
...
from surveillance_static_phone_suppression import (
    ...
    load_desk_static_zones_from_calibration,
)
...
class PersonPose:
    ...
    l_elbow: Optional[tuple[float, float]]
    r_elbow: Optional[tuple[float, float]]
...
class CandidateEval:
    ...
    best_angle: float
    best_ray_hit: bool
    person_expand_roi: Optional[Rect] = None
    hand_rois: list[Rect] = field(default_factory=list)
    corridor_bbox: Optional[Rect] = None
    effective_search_roi: Optional[Rect] = None
    track_id: int = -1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--enable-static-phone-suppression", dest="enable_static_phone_suppression", action="store_true", default=True)
    parser.add_argument("--disable-static-phone-suppression", dest="enable_static_phone_suppression", action="store_false")
    parser.add_argument("--static-window-seconds", type=float, default=1.5)
    parser.add_argument("--static-max-disp-ratio", type=float, default=0.03)
    parser.add_argument("--static-risk-multiplier", type=float, default=0.2)
    parser.add_argument("--desk-zone-field", default="desk_static_zones")
...
def union_boxes(boxes: list[Rect]) -> Optional[Rect]:
...
def intersect_boxes(a: Rect, b: Rect) -> Optional[Rect]:
...
def compute_temporal_score(state: Optional[PersonTrackState], risk_threshold: float) -> float:
...
def update_eval_risk(ev: CandidateEval) -> None:
    ...
    ev.final_risk_score = ev.risk_score
    if ev.person is not None and ev.reject_reason in ("", "low_joint_score", "low_risk_score"):
        if ev.risk_score > 0.65 and ev.static_zone_score > 0.0 and ev.person_match_score >= 0.35:
            ev.reject_reason = ""
        else:
...
def update_person_states(
    ...
    for person in people:
        ...
        static_suppressed = ev is not None and ev.static_suppressed
        risk = ev.risk_score if ev is not None else 0.0
        phone_seen = ev is not None and ev.phone_score > 0.0
        candidate = ev is not None and ev.accepted and not static_suppressed and ev.risk_score >= risk_threshold
        state.bbox_history.append(person.box)
        if static_suppressed:
        ...
        candidate_ratio = sum(state.candidate_history) / max(1, len(state.candidate_history))
        avg_risk = sum(state.risk_history) / max(1, len(state.risk_history))
        enough_history = len(state.candidate_history) >= max(1, min_hits)
        state.alarm_triggered = (not static_suppressed) and enough_history and candidate_ratio > 0.60 and avg_risk > 0.70
        if state.alarm_triggered:
            state.state = "S4_ALARM"
        elif candidate_ratio > 0.40 and avg_risk > 0.50:
...
def get_point(kp, idx: int, conf: float) -> Optional[tuple[float, float]]:
...
def draw_poly(frame, poly: list[tuple[float, float]], color: tuple[int, int, int], thickness: int, label: Optional[str] = None) -> None:
...
def draw_rect(frame, rect: Rect, color: tuple[int, int, int], thickness: int, label: Optional[str] = None) -> None:
...
def build_effective_search_roi(person: PersonPose, screen: ScreenConfig, width: int, height: int) -> tuple[Optional[Rect], Rect, list[Rect], Rect]:
    p = screen.params
...
def match_phone_to_person(phone: PhoneCandidate, persons: list[PersonPose], screen: ScreenConfig, width: int, height: int) -> tuple[Optional[PersonPose], float, float, Optional[Rect], list[Rect], Optional[Rect], bool]:
...
def evaluate_phone(phone: PhoneCandidate, screen: ScreenConfig, persons: list[PersonPose], width: int, height: int) -> CandidateEval:
    ...
    if zone_reason.startswith("ignore"):
        ...
        if person is None:
            ...
            if risk <= 0.65 or person_match_score < 0.35:
...
def _nearest_wrist_point(ev: CandidateEval) -> Optional[tuple[float, float]]:
    if ev.person is None:
        return None
    choices: list[tuple[float, tuple[float, float]]] = []
    if ev.person.l_wrist is not None and ev.phone_to_left_wrist_dist >= 0:
        choices.append((ev.phone_to_left_wrist_dist, ev.person.l_wrist))
    if ev.person.r_wrist is not None and ev.phone_to_right_wrist_dist >= 0:
        choices.append((ev.phone_to_right_wrist_dist, ev.person.r_wrist))
    if not choices:
        return None
    return min(choices, key=lambda item: item[0])[1]


def apply_static_phone_suppression(
    evals: list[CandidateEval],
    static_state: StaticPhoneSuppressionState,
    camera_id: str,
    frame_id: int,
    fps: float,
    desk_static_zones: list[list[tuple[float, float]]],
    config: StaticPhoneConfig,
) -> None:
    if not config.enabled:
        for ev in evals:
            ev.final_risk_score = ev.risk_score
        return
    timestamp = frame_id / max(float(fps), 1e-6)
    for ev in evals:
        ev.final_risk_score = ev.risk_score
        if ev.person is None or ev.track_id < 0:
            continue
        obs = StaticPhoneObservation(
            camera_id=camera_id,
            person_id=ev.track_id,
            frame_id=frame_id,
            timestamp=timestamp,
            phone_center=ev.phone.center,
            phone_bbox=_rect_tuple(ev.phone.box),
            person_bbox=_rect_tuple(ev.person.box),
            nearest_wrist=_nearest_wrist_point(ev),
            risk_score=ev.risk_score,
            candidate=ev.accepted and ev.risk_score > 0.65,
        )
        result = static_state.update(obs, desk_static_zones=desk_static_zones)
        ev.phone_static = result.phone_static
        ev.phone_static_duration = result.phone_static_duration
        ev.phone_motion_px = result.phone_motion_px
        ev.phone_in_desk_zone = result.phone_in_desk_zone
        ev.static_suppressed = result.static_suppressed
        ev.static_risk_multiplier = result.static_risk_multiplier
        ev.phone_follow_wrist = result.phone_follow_wrist
        if result.static_suppressed:
            ev.risk_score = clamp(ev.risk_score * config.risk_multiplier, 0.0, 1.0)
            ev.final_risk_score = ev.risk_score
            ev.level = candidate_level_from_risk(ev.risk_score)
            ev.reject_reason = "static_phone_suppressed"
            ev.candidate_reason = (
                f"STATIC PHONE SUPPRESSED|duration={result.phone_static_duration:.2f}s|"
                f"motion={result.phone_motion_px:.1f}px|risk={ev.risk_score:.2f}"
            )
...
def write_debug_row(writer: csv.DictWriter, frame_id: int, ev: CandidateEval, alert_counter: float, stable_alert: bool, effective_roi: Optional[Rect]) -> None:
    p = ev.person
    phone = ev.phone
    writer.writerow(
        {
            ...
            "phone_static": ev.phone_static,
            "phone_static_duration": f"{ev.phone_static_duration:.3f}",
            "phone_motion_px": f"{ev.phone_motion_px:.2f}",
            "phone_in_desk_zone": ev.phone_in_desk_zone,
            "static_suppressed": ev.static_suppressed,
            "static_risk_multiplier": f"{ev.static_risk_multiplier:.3f}",
            "phone_follow_wrist": ev.phone_follow_wrist,
            ...
            "final_risk_score": f"{ev.final_risk_score:.3f}",
...
def draw_outputs(frame, screens: list[ScreenConfig], people: list[PersonPose], evals: list[CandidateEval], person_states: dict[int, PersonTrackState], alert_counter: float, stable_alert: bool, draw_static_zones: bool, draw_mode: str = "full", desk_static_zones: Optional[list[list[tuple[float, float]]]] = None) -> None:
    ...
    if draw_static_zones and not compact and desk_static_zones:
        for idx, poly in enumerate(desk_static_zones, start=1):
            draw_poly(frame, poly, (150, 150, 150), 1, f"DESK_STATIC:{idx}")
    for person in people:
    ...
    for ev in evals:
        if ev.static_suppressed:
            color = (150, 150, 150)
        elif ev.accepted and ev.level == "strong":
            color = (0, 0, 255)
        elif ev.accepted:
            color = (0, 165, 255)
            ...
            color = (120, 120, 120)
        if compact and not ev.accepted and not ev.static_suppressed:
            continue
        phone_label = "STATIC" if ev.static_suppressed else ("PHONE" if compact else f"{ev.phone.source}:{ev.phone.conf:.2f}")
        draw_rect(frame, ev.phone.box, color, 3 if ev.accepted else 1, phone_label)
        if not compact:
            ...
            label = "STATIC PHONE SUPPRESSED" if ev.static_suppressed else (ev.candidate_reason if ev.accepted else ev.reject_reason)
...
def main() -> None:
    ...
    desk_static_zones = load_desk_static_zones_from_calibration(args.screen_config, width, height, args.desk_zone_field)
    static_phone_config = StaticPhoneConfig(
        enabled=bool(args.enable_static_phone_suppression),
        window_seconds=float(args.static_window_seconds),
        max_disp_ratio=float(args.static_max_disp_ratio),
        risk_multiplier=float(args.static_risk_multiplier),
    )
    static_phone_state = StaticPhoneSuppressionState(static_phone_config)
    ...
    stats = {
        ...
        "max_risk_score": 0.0,
        "static_phone_suppressed_frames": 0,
        "static_phone_suppressed_candidates": 0,
        "desk_zone_phone_frames": 0,
        "tracked_persons": 0,
    ...
    fields = [
        ...
        "phone_in_person_roi",
        "phone_static",
        "phone_static_duration",
        "phone_motion_px",
        "phone_in_desk_zone",
        "static_suppressed",
        "static_risk_multiplier",
        "phone_follow_wrist",
    ...
    with debug_csv.open("w", newline="", encoding="utf-8") as f:
        ...
        while True:
            ...
            apply_static_phone_suppression(evals, static_phone_state, camera_id, frame_id, fps, desk_static_zones, static_phone_config)
            ...
            draw_outputs(annotated, screens, people, evals, person_states, alert_counter, stable_alert, args.draw_static_zones, args.draw_mode, desk_static_zones)
            ...
            stats["static_phone_suppressed_frames"] += int(any(e.static_suppressed for e in evals))
            stats["static_phone_suppressed_candidates"] += sum(1 for e in evals if e.static_suppressed)
            stats["desk_zone_phone_frames"] += int(any(e.phone_in_desk_zone for e in evals))
    ...
    writer.release()
    summary = {
        "version": "v2.3_static_phone_suppression",
        "camera_id": camera_id,
